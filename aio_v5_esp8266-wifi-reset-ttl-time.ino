...
SoftwareSerial mySerial(0, 2);  D3RX, D4TX
...
void loop() {
  ...
  if (newData == true) {
     ...
     switch(select) {
      case 1 {
        ...
        scani2c();
           ...
           Serial.println(F(3.write or read i2c wr,addr,reg,value)); 
           ...
           client.println(F(3.write or read i2c wr,addr,reg,value)); 
    ...
    if ( strcmp(opCode,d) == 0 ) {
        ...
        Serial.println(F(D3RX D4TX));
        client.println(F(D3RX D4TX));
        display.println(F(D3RX D4TX));
        clearscreen();
...
void ttlsetup ( int BAUD ) {
 ...
 while(digitalRead(buttonPin) == HIGH) {
  ...
  if (client) {
    Serial.println(TTL modeTCP client connected);
    client.println(TTL modeTCP client connected);
